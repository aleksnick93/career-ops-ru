...
- **Сканирует сайты поиска вакансий** — hh.ru, Habr Career, getmatch.ru + международные Greenhouse, Ashby, Lever
...
| **Российские компании** | Yandex, Sber AI, Tinkoff Tech, VK, Avito, Kaspersky и другие |
...
| **Белая / серая зарплата** | Серая зарплата = red flag в скоринге (трудности в одобрении кредитов, отсутствие пенсионного стажа) |
| **ДМС** | Ключевой бенефит в РФ. Отсутствие ДМС в крупной компании снижает оценку |
| **Gross / Net (НДФЛ 13%)** | В объявлениях часто указывают gross. Система пересчитывает: net = gross × 0.87 |
| **Испытательный срок** | 3 месяца - норма. Больше для линейных специалистов - флаг |
| **IT-аккредитация Минцифры** | Льготная ипотека, отсрочка от армии, пониженные страховые взносы - сильный плюс в оценке |
...
Сканер включает **55+ компаний** и **19 поисковых запросов** по основным российским и международным площадкам. Скопируй ```templates/portals.example.yml``` в ```portals.yml``` и добавь свои собственные
...
### Площадки для поиска
**Российские:** HH, Хабр.Карьера, GetMatch
...
│   ├── ru/                      # Русские модули (по умолчанию)
│   │   ├── _shared.md           # Контекст + специфика RU-рынка
│   │   ├── vakansiya.md         # Оценка вакансии (A-F)
...
├── dashboard/                   # Go дашборд в терминале
├── data/                        # Данные трекера
